...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await api.downloadBulkResults(
      validatedFilters.fileId,
      validatedFilters.filter as any,
      {
    ...
    if (error instanceof Error && error.name === 'ZodError') {
      return NextResponse.json(
        { error: 'Invalid download parameters', details: error.message },
        { status: 400 }
      );
    }
